...
- 课表：状态按秒推送，七日课表单独低频同步；插件设置页可手动推送当前课表，WebUI 与手表课表页可随时强制拉取并显示进度，成功后以插件最新课表覆盖服务端旧缓存；插件推送、WebUI 拉取、手表拉取、自动拉取和连接初始化共享同一个任务锁，运行中会在各端显示来源并拒绝重复任务；WebUI 还可设置 15 分钟、1 小时、6 小时或每天的刷新周期；支持指定日期交换两节课或替换科目，并用修订号防止并发覆盖。
