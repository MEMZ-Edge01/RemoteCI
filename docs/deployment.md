...
插件每次连接云端后，服务端都会立即请求一次最新七日课表。如果 WebUI“七日课表”仍提示尚未同步，可点击“立即向插件拉取课表”；同一页面还可把在线期间的自动拉取周期设为关闭、每 15 分钟、每小时、每 6 小时或每天，选择会保存在 SQLite 中并在重启后保留。插件离线时不会排队，重新连接后会按连接机制再次拉取。
